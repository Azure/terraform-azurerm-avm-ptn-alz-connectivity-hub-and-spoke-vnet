locals {
  hub_virtual_networks = nonsensitive(sensitive(var.hub_virtual_networks))
}
...
locals {
  ...
  hub_virtual_networks_resource_group_names = { for key, value in local.hub_virtual_networks : key => provider::azapi::parse_resource_id("Microsoft.Resources/resourceGroups", coalesce(value.default_parent_id, value.hub_virtual_network.parent_id)).resource_group_name }
  primary_location                          = local.has_regions ? local.hub_virtual_networks[local.primary_region_key].location : null
  primary_region_key                        = local.has_regions ? keys(local.hub_virtual_networks)[0] : null
}
