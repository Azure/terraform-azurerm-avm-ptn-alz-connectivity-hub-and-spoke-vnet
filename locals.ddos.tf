locals {
  ...
  ddos_protection_plan_name                = coalesce(var.hub_and_spoke_networks_settings.ddos_protection_plan.name, local.default_names[local.primary_region_key].ddos_protection_plan_name)
