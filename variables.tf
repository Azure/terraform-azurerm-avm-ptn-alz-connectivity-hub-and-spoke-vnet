...
- `hub_virtual_network` - (Optional) An object defining the hub virtual network settings. The object has the following fields:
  ...
  - `subnets` - (Optional) A map of subnets to create in the virtual network. The value is an object with the following fields:
    ...
    - `service_endpoints` - (Optional) A set of Service Endpoint service names (for example `["Microsoft.KeyVault", "Microsoft.Storage"]`) to associate with the subnet. This is the supported way to configure Service Endpoints.
    - `service_endpoints_with_location` - (Optional, **removed**) No longer supported. Setting a non-null value fails validation. Use `service_endpoints` (a set of service names) instead; per-service locations are not configurable because Azure expands service endpoint locations implicitly.
...
DESCRIPTION
  ...
  validation {
    condition = alltrue([
      for hub_key, hub in var.hub_virtual_networks :
      hub.dns_resolver_policy == null || try(hub.dns_resolver_policy.parent_id, null) == null ? true :
      can(provider::azapi::parse_resource_id("Microsoft.Resources/resourceGroups", hub.dns_resolver_policy.parent_id))
    ])
    error_message = "When set, hub_virtual_networks[*].dns_resolver_policy.parent_id must be a valid resource group resource ID (e.g. /subscriptions/<sub>/resourceGroups/<rg>)."
  }
  validation {
    condition = alltrue([
      for hub_key, hub in var.hub_virtual_networks :
      hub.dns_resolver_policy == null ? true : alltrue([
        for _, link in hub.dns_resolver_policy.additional_virtual_network_links :
        can(provider::azapi::parse_resource_id("Microsoft.Network/virtualNetworks", link.virtual_network_id))
      ])
    ])
    error_message = "hub_virtual_networks[*].dns_resolver_policy.additional_virtual_network_links[*].virtual_network_id must be a valid Microsoft.Network/virtualNetworks resource ID."
  }
  validation {
    condition = alltrue([
      for hub_key, hub in var.hub_virtual_networks :
      hub.dns_resolver_policy == null ? true : alltrue([
        for _, rule in hub.dns_resolver_policy.rules :
        contains(["Alert", "Allow", "Block"], rule.action) && contains(["Enabled", "Disabled"], rule.state)
      ])
    ])
    error_message = "hub_virtual_networks[*].dns_resolver_policy.rules[*].action must be one of Alert/Allow/Block and .state must be Enabled/Disabled."
  }
  validation {
    condition = alltrue([
      for hub_key, hub in var.hub_virtual_networks :
  ...
  validation {
    ...
    error_message = "`service_endpoints_with_location` is no longer supported. Set `service_endpoints` to a set of service names instead, for example [\"Microsoft.Storage\"]. Per-service locations are not configurable because Azure expands service endpoint locations implicitly."
  }
}
