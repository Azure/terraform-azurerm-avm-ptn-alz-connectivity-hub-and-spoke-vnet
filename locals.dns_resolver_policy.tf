locals {
  ...
  dns_resolver_policy_enabled = { for key, value in local.hub_virtual_networks_by_key :
    key => nonsensitive(value.enabled_resources.dns_resolver_policy && value.dns_resolver_policy != null)
  }
}
...
locals {
  ...
  dns_resolver_policy = { for key, value in local.hub_virtual_networks_by_key : key => {
    ...
    domain_lists = {
      for dl_key, dl in { for domain_list_key in nonsensitive(keys(value.dns_resolver_policy.domain_lists)) : domain_list_key => value.dns_resolver_policy.domain_lists[domain_list_key] } : dl_key => {
    ...
    security_rules = { for rule_key in nonsensitive(keys(value.dns_resolver_policy.rules)) : rule_key => value.dns_resolver_policy.rules[rule_key] }
    virtual_network_links = merge(
      nonsensitive(value.dns_resolver_policy.link_to_hub_virtual_network) ? {
        hub = {
      ...
      { for link_key in nonsensitive(keys(value.dns_resolver_policy.additional_virtual_network_links)) : link_key => value.dns_resolver_policy.additional_virtual_network_links[link_key] },
    )
