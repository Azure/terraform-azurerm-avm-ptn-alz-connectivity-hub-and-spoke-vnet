locals {
  firewall_default_ip_configuration = {
    for key, value in local.hub_virtual_networks : key => merge(value.firewall.default_ip_configuration, {
  ...
  firewall_enabled = { for key, value in local.hub_virtual_networks : key => value.enabled_resources.firewall }
  firewall_ip_configurations = {
    for key, value in local.hub_virtual_networks : key => merge(value.firewall.ip_configurations, {
  ...
  firewall_management_ip_configuration = {
    for key, value in local.hub_virtual_networks : key => merge(value.firewall.management_ip_configuration, {
  ...
  firewall_policies = { for key, value in local.hub_virtual_networks : key => local.firewall_enabled[key] ? merge(value.firewall_policy, {
  ...
  firewall_policy_dns_defaults = { for key, value in local.hub_virtual_networks : key => local.private_dns_resolver_enabled[key] && local.private_dns_zones_enabled[key] && local.firewall_enabled[key] && !local.firewall_sku_is_basic[key] && value.private_dns_resolver.default_inbound_endpoint_enabled ? {
    proxy_enabled = true
    servers       = [local.private_dns_resolver_ip_addresses[key]]
  } : null }
  firewall_sku_is_basic = { for key, value in local.hub_virtual_networks : key => local.firewall_enabled[key] && (value.firewall.sku_tier == "Basic" || value.firewall_policy.sku == "Basic") }
  firewalls = { for key, value in local.hub_virtual_networks : key => local.firewall_enabled[key] ? merge(value.firewall, {
