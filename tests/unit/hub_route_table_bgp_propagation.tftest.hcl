...
# hub_route_table_bgp_propagation_enabled setting and that
...
run "bgp_route_propagation_can_be_disabled" {
  ...
  variables {
    ...
    hub_virtual_networks = {
      primary = {
        ...
        hub_route_table_bgp_propagation_enabled = false
  ...
  assert {
    ...
    error_message = "Hub user subnet route table must disable BGP route propagation when hub_route_table_bgp_propagation_enabled is false."
  }
}
...
run "bgp_route_propagation_can_be_enabled" {
  ...
  variables {
    ...
    hub_virtual_networks = {
      primary = {
        ...
        route_table_user_subnets_enabled        = true
        hub_route_table_bgp_propagation_enabled = true
        hub_router_ip_address                   = "10.0.0.4"
      }
    }
  }
  ...
  assert {
    condition     = module.hub_routing_user_subnets["primary"].resource.bgp_route_propagation_enabled == true
    error_message = "Hub user subnet route table must enable BGP route propagation when hub_route_table_bgp_propagation_enabled is true."
  }
}
