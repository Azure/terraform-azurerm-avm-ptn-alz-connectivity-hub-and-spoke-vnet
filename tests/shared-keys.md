...
The existing workflow's fork-PR restriction is unchanged: it only invokes the reusable workflow for non-fork PRs. These unit tests themselves use mocked Azure providers and synthetic values, without deploying Azure resources. Initial setup downloads the managed tools, modules and providers.
...
| Unknown keys before JSON/template processing | Plan-only direct/actual-example fixtures, known-identity assertions and non-sensitive re-exports | No generated unknown keys |
