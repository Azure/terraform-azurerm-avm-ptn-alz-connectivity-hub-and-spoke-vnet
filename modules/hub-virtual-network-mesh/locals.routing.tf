# Route Tables
locals {
  create_route_tables_firewall     = { for key, value in local.hub_virtual_networks_by_key : key => nonsensitive(value.route_table_firewall_enabled && (value.firewall != null || value.hub_router_ip_address != null)) }
  create_route_tables_user_subnets = { for key, value in local.hub_virtual_networks_by_key : key => nonsensitive(value.route_table_user_subnets_enabled && (value.firewall != null || value.hub_router_ip_address != null)) }
  route_tables_firewall            = { for key, value in local.hub_virtual_networks_by_key : key => value if local.create_route_tables_firewall[key] || nonsensitive(length(value.route_table_entries_firewall) > 0) }
  route_tables_user_subnets        = { for key, value in local.hub_virtual_networks_by_key : key => value if local.create_route_tables_user_subnets[key] || nonsensitive(length(value.route_table_entries_user_subnets) > 0) }
}
