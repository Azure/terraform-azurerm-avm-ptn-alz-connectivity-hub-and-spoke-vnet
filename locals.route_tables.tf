locals {
  gateway_route_table = { for key, value in var.hub_virtual_networks : key => {
    ...
    # Will throw error in plan if gw subnet does not exist yet, therefor need to make sure value passed does not trigger error in sub module
    subnet_resource_ids           = can(module.hub_and_spoke_vnet.virtual_networks[key].subnets["${key}-gateway"]) ? module.hub_and_spoke_vnet.virtual_networks[key].subnets["${key}-gateway"].resource_id != null ? { gw-subnet = module.hub_and_spoke_vnet.virtual_networks[key].subnets["${key}-gateway"].resource_id } : {} : {}
