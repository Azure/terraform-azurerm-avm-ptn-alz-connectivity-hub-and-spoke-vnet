locals {
  ...
  gateway_route_table_routes = { for key, value in var.hub_virtual_networks : key => {
    for routeKey, route in value.virtual_network_gateways.routes : routeKey => {
      name                   = null(route.name) ? "${key}-${routeKey}-${replace(route.address_prefix, "/", "-")}" : route.name
      address_prefix         = route.address_prefix
      next_hop_type          = null(route.next_hop_type) ? "VirtualAppliance" : route.next_hop_type
      next_hop_in_ip_address = null(route.next_hop_in_ip_address) ? try(module.hub_and_spoke_vnet.firewalls[key].private_ip_address, null) : route.next_hop_in_ip_address
    }
    }
  }
