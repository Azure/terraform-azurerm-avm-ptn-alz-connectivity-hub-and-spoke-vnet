locals {
  gateway_route_table_routes = { for key, value in var.hub_virtual_networks : key => {
    for routeKey, route in value.routes : routeKey => {
      ...
      # Data lookup creates cyclic error
      next_hop_in_ip_address = can(route.next_hop_in_ip_address) ? route.next_hop_in_ip_address : try(data.azurerm_firewall.azfws[key].ip_configuration.private_ip_address, null)
    }
    }
  }
}
...
#   }
  ...
  gateway_route_table = { for key, value in var.hub_virtual_networks : key => {
    ...
    # Usage of local.virtual_network_gateways[key].virtual_network_gateway_subnet_id creates cyclic error
    # subnet_resource_ids = can(local.virtual_network_gateways[key].virtual_network_gateway_subnet_id) ? { default = local.virtual_network_gateways[key].virtual_network_gateway_subnet_id } : {}
