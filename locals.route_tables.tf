locals {
  gateway_route_table = { for key, value in local.hub_virtual_networks : key => {
  ...
  gateway_route_table_enabled = { for key, value in local.hub_virtual_networks : key => (local.virtual_network_gateways_express_route_enabled[key] || local.virtual_network_gateways_vpn_enabled[key]) && value.virtual_network_gateways.route_table_creation_enabled }
}
