locals {
  gateway_route_table = { for key, value in var.hub_virtual_networks : key => {
    ...
    subnet_resource_ids           = can(module.hub_and_spoke_vnet.virtual_networks[key].subnets["${key}-gateway"].resource_id) ? { gw-subnet = module.hub_and_spoke_vnet.virtual_networks[key].subnets["${key}-gateway"].resource_id } : {gw-subnet = null}
